    ...
    <div data-role="collapsible-set" data-inset="false">
      ...
      <div data-role="collapsible">
		...
		{	
		  		...
		  		$havemenu = $menu->have_time_menu($row['id'],$availabledays);
		  		$topproduct = $menu->productnameByCat($row['id'],$availabledays,$havemenu['have_time_menu']);
			    if($topproduct){				

				foreach($topproduct as $tprow){
				 if($prow['sizedid']!=0)
				 {
					$sizedId = "&sizedid=".$tprow['sizedid']."";
				 }
				 else
					{
						$sizedId="";
				  ...
				  ?>
				  <li><a  data-ajax="false" href="toppingdetails.php?Item=<?php echo $tprow['id'];?><?php echo $sizedId;?>">	
				  <?php
				   if(isset($tprow['thumb_item_image']) && $tprow['thumb_item_image']!='' && is_file("../uploads/menuitem/thumb/".$tprow['thumb_item_image']))
					{ ?>
					<img src="<?php echo UPLOADURL;?>/menuitem/thumb/<?php echo $tprow['thumb_item_image'];?>"  width="50" height="50" />
					<?php }			  

				   echo $tprow['item_name'];?></a></li>
				  <?php }	
				   }
            ...
            <ul data-role="listview" data-inset="false">
		  		<?php 
		  		$havemenu = $menu->have_time_menu($srow['id'],$availabledays);
		  		$product = $menu->productnameByCat($srow['id'],$availabledays,$availabilityTime,$havemenu['have_time_menu']);
