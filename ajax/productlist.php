        ...
        <h4 class="product-cat"><?php echo $crow['category_name'];?></h4> 
         <?php 
		 	if(isset($crow['category_dec']) && !empty($crow['category_dec'])){
				echo '<p class="category_desc">'.cleanOut(ucwords($crow['category_dec'])).'</p>';
			}  
		?>       
        <div class="row-fluid">
        <?php  
		 ...
		 ?>
          <div class="span12 fit">
            <div class="span12 product-listing">
              <h4><a href="#category-<?php echo $crow['id']."-". paranoia($catname)."/product-". $prow['id'].$sizedurl;?>" id="clickingEvent" class="itemtopping" rel="<?php echo $prow['id'];?>"><?php echo $prow['item_name'];?></a>
               <?php 
...
{ 
			 ...
			 $availabilityTime =  $menu->OrderAvailabilityTime($_GET['locationid'],$orerdady,$ordertime);
 			 $availabledays = $_GET['availabledays'];
			$category = $menu->categotyfront($_GET['locationid']);
			if($category):			
...
?> 
        ...
        <div class="row-fluid">
      <?php   
	  $havemenu = $menu->have_time_menu($crow['id'],$availabledays);
	  $product = $menu->productnameByCat($crow['id'],$availabledays,$availabilityTime,$havemenu['have_time_menu']);	   
	  if($product):
			$sizedurl = "";
			foreach($product as $prow):
	  ...
	  ?>
          <div class="span12 fit">
            <div class="span12 product-listing">
              <h4>
              <a href="#category-<?php echo $crow['id']."-". paranoia($catname)."/product-". $prow['id'].$sizedurl;?>" id="clickingEvent" class="itemtopping" rel="<?php echo $prow['id'];?>"><?php echo $prow['item_name'];?></a>
              <?php 
			  if($prow['special_menu_icon'])
			  {
			  	$SpecialMenuIcon = $menu->SpecialMenuIcon($prow['special_menu_icon']);
