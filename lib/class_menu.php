<?php
 ...
 /************************* Front end function start here************************************************************ */
	  ...
	  */
	   ...
	   * working fine
	   * @return single image upload
	   */
	  public function processMenuOptionTest_Previous()
	  {
		...
		/*  if (empty($_FILES['fileinput']['name']))
			  $core->msgs['fileinput'] = "provide image";			
			  
		  if (!empty($_FILES['fileinput']['name'])) {
			  if (!preg_match("/(\.jpg|\.png|\.gif)$/i", $_FILES['fileinput']['name'])) {
				  $core->msgs['fileinput'] = "provide valid image";
			  }
			  $file_info = getimagesize($_FILES['fileinput']['tmp_name']);
			  if(empty($file_info))
				  $core->msgs['fileinput'] = "provide valide image";
		  }*/
		  ...
		  if (empty($core->msgs)) {
			  ...
			  $newName = "IMG_" . randName();
			  $ext = substr($_FILES['fileinput']['name'], strrpos($_FILES['fileinput']['name'], '.') + 1);
			  $name = $newName.".".strtolower($ext);		
			  $als = new Upload();
			  $als->File = $_FILES['fileinput'];
			  $als->method = 1;
			   $als->SavePath = UPLOADS.'/choice_images/';
			  //$als->NewWidth = $core->project_logo_w= 151; 
			  //$als->NewHeight = $core->project_logo_h=57;
			  $als->NewName  = $newName;
			  $als->OverWrite = true;
			  $err = $als->UploadFile();
			  ...
			  if ($this->menuid) {
				  ...
				  if (!empty($_FILES['fileinput']['name'])) {
					  ...
					  $data['choice_image'] = $name;
				  } else {
					  $data['choice_image'] = $avatar;
				  }
			  } else {
				  if (!empty($_FILES['fileinput']['name'])) 
				  $data['choice_image'] = $name;
			  ...
			  ($this->menuid) ? $db->update("res_menu_option_choice_master", $data, "id='" . (int)$this->menuid . "'") : $db->insert("res_menu_option_choice_master", $data);
			  $message = ($this->menuid) ? "Image updated" : "Image added";

			  if ($db->affected()) {
				  $core->msgOk($message);
				  $wojosec->writeLog($message, "", "no", "Map");
	   ...
	   * Menu::processMenuOptionTest()
	   * working fine
	   * @return single image upload working fine
	   */
	  public function processMenuOptionTest()
	  {
		  global $db, $core, $wojosec;
		  
		  print_r($_FILES['fileinput']['name']);		  
		  $arr_count = count($_FILES['fileinput']['name']); 
	 ...
	 for($i=0; $i<$arr_count; $i++){
			  ...
			  $data = array( 
			   ...
			   // Start Avatar Upload
			  $newName = "IMG_" . randName();
			  $ext = substr($_FILES['fileinput']['name'][$i], strrpos($_FILES['fileinput']['name'][$i], '.') + 1);
			  $name = $newName.".".strtolower($ext);		
			  $als = new Upload();
			  $als->File = $_FILES['fileinput'];
			  $als->method = 1;
			...
			$db->insert("res_menu_option_choice_master", $data);  
	 }
	...
	/**
	 ...
	 /**
	  ...
	  public function processMenuItemFromXML($data = array())
	  {
			...
			return $db->insert("res_menu_item_master", $idata);
	  }  
	  /**
	  ...
	  {  
		  global $db, $core, $wojosec;	  
			...
			return $db->insert("res_menu_size_master", $data);
	  }
	  /**
	  ...
	  public function processMenuItemSizeMappingFromXML($data = array())
	  {
			...
			return $db->insert("res_menu_size_mapping", $idata);
	  }
	  ...
	  {  
		  global $db, $core, $wojosec;	
			...
			return $db->insert("res_category_master", $data);
			$insert_id = $db->insertid();	
			if(!empty($insert_id))
			{
