<?php
  ...
  class Product
  {
	...
	*/
	...
	{
		global $db, $core;
		$sql = "SELECT `id`,`is_cash_on_delivery`,`is_paypal`,`is_authorize`,`is_first_data`,`is_mercury`,`is_internet_secure`,`merchant_id`,`merchant_password`,is_vm,vm_merchant_id,vm_user_id,vm_pin"
  ...
  $order_details2 ='<tr>
  					<td style="color:#000000; font-size:11px; font-family:Verdana, Arial, Helvetica, sans-serif;">
						<table width="437" border="0" cellspacing="0" cellpadding="0">
						  <tr style="background:#ed1d24">
							<td style="background:#ed1d24;color:#fff;font-size:12px;font-weight:bold;font-family:Verdana,Arial,Helvetica,sans-serif;padding-left:8px; line-height:20px;">Quantity </td>
							<td style="background:#ed1d24;color:#fff;font-size:12px;font-weight:bold; font-family:Verdana, Arial, Helvetica, sans-serif;line-height:20px; padding-left:10px;">Description</td>
							<td style="background:#ed1d24;color:#fff;font-size:12px;font-weight:bold;font-family:Verdana,Arial, Helvetica,sans-serif;line-height:20px;text-align:left; padding-right:10px;">Amount</td>
					</tr>';			
		...
		if($topping){	
			foreach ($topping as $trow){ 
				$optionChoiceName = ($trow['choice_name'])? $trow['choice_name'] : '';
     		    ...
     		    $order_details2.='<span style="color:#ef4423; font-weight:bold; font-size:11px; font-family:Verdana, Arial, Helvetica, sans-serif;">'.$trow['option_topping_name'].'</span>'.$optionChoiceName2.' <br />';
        ...
        <td style="border-bottom:1px solid #231f20; padding-bottom:20px;" colspan="3"><table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
              ...
              <td style="padding:10px;">
			  	<table width="100%" border="0" cellspacing="0" cellpadding="0"  style="background:#ed1d24;font-size:12px;color:#fff">';
...
$order_details2.='</table></td>
      ...
      $order_details2.=' </table>
						  ...
						  if(isset($data['order_type']) && !empty($data['order_type']) && $data['order_type']=='d'){
							   $delivery_address = ($data['apt']) ? '<span style="color:#602c0f; font-size: 11px; font-family: Verdana,Arial,Helvetica,sans-serif;"> Delivery Address: </span><span style="color:black; font-size:11px; font-family:Verdana, Arial, Helvetica, sans-serif;">'.$data['apt'].',' : '';
						  ...
						  if(isset($data['order_type']) && !empty($data['order_type']) && $data['order_type']=='d'){ 
							   $delivery_dateTime = '<span style="color:#602c0f; font-size: 11px; font-family: Verdana,Arial,Helvetica,sans-serif;"> Delivery Time: </span><span style="color:black; font-size:11px; font-family:Verdana, Arial, Helvetica, sans-serif;">'.$order_dt. '  '.$order_tm.'</span>';  
						 ...
						 // print html_entity_decode($body); exit();				  	  				  			  			 				 
				  		  $mailer = $mail->sendMail();
						  $message = Swift_Message::newInstance()
						   ...
						   $mailer->send($message);
							  ...
							  $order_details3 = str_replace(array('background:#ed1d24','color:#fff','color:#ef4423;','font-weight:bold; font-size:11px; font-family:Verdana, Arial, Helvetica, sans-serif;'),array('','','','font-size:11px; font-family:Verdana, Arial, Helvetica, sans-serif;'),$order_details2);	
