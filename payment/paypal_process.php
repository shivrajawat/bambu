  ...
  if (!$customers->customerlogged_in)
      redirect_to("signin");	  		
			...
			if($_SESSION['access_type']=='c'){
				$PayPalReturnURL 		= SITEURL.'/payment/paypal_process.php'; //Point to process.php page
				$PayPalCancelURL 		= SITEURL.'/checkout'; //Cancel URL if user clicks cancelexit();
			} else{
				$PayPalReturnURL 		= SITEURL.'/payment/paypal_process.php'; //Point to process.php page
  ...
  if (isset($_POST['processPaymnet']) && $_POST['patment_type']=='is_paypal' ){		
	...
	$padata = 	'&CURRENCYCODE='.urlencode($PayPalCurrencyCode).
		...
		{					
			...
			header( "refresh:5;url=".SITEURL."/checkout.php" );
		}
