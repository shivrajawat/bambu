...
if($row)
{
	...
	$notes .= Special_Char_convert($row['order_comments']);
	...
	if($orderproductrow)
	{
		foreach($orderproductrow as $prow)
		{
			...
			if($prow['item_name'])
			{
				$name = removespecialcharacters($prow['item_name']);
				
				CreateElementAttrWithValue($dom, $ticket_item, "name", Special_Char_convert($name), false);
			}
			...
			if($prow['item_type'] == 1)
			{
				...
				$size_name = removespecialcharacters($prow['size_name']);
				
				CreateElementAttrWithValue($dom, $size, "name", Special_Char_convert($size_name), false);
				//attribute price creator
			...
			if(($prow['item_type'] == 1) && (in_array($MenuType, array('calculate', 'topping', 'sauce', 'cheese'))))
			{
				...
				//attribute id creator
				$split_name = removespecialcharacters($prow['item_name']);
				CreateElementAttrWithValue($dom, $split, "name", Special_Char_convert($split_name), false);
			...
			if($toppingrow)
			{
				...
				if(($OptionType == 'r') || ($OptionType == 'd'))
				{
					...
					{
						$modifier_name = removespecialcharacters($trow['topping_name']);
					}
					CreateElementAttrWithValue($dom, $modifier, "name", Special_Char_convert($modifier_name), false);
					//attribute price creator
					if($MenuType == 'modifier')
					{
				...
				if($OptionType == 'c')
				{
					...
					if(($MenuType != 'crust') && ($MenuType != 'modifier'))
					{
						$modifier_location = removespecialcharacters($trow['choice_name']);
						CreateElementAttrWithValue($dom, $modifier, "location", Special_Char_convert($modifier_location), false);
					}
	...
	if($row['customer_id'])
	{
		...
		if($crow['first_name'])
		{
			$first_name = Special_Char_convert($crow['first_name']);
			CreateElementWithValue($dom, $customer, "first_name", $first_name, false);
		}
		//first name creator
		//if($crow['last_name'])
		//{
			$last_name = Special_Char_convert($crow['last_name']);
		...
		//{
			...
			CreateElementWithValue($dom, $customer, "addr", Special_Char_convert($addr), false);
		//}
		//city creator
		//if($crow['city_id'])
		//{
			...
			//}
			$city = trim($crow['city']);
			CreateElementWithValue($dom, $customer, "city", Special_Char_convert($city), false);
		//}
		//state creator
		//if($crow['state_id'])
//		{
...
//		}
		...
		CreateElementWithValue($dom, $customer, "state", Special_Char_convert($state), false);
		//postal creator
		//if($crow['zipcode'])
//		{
		...
		//{
			...
			CreateElementWithValue($dom, $customer, "email", Special_Char_convert($email), false);
		//}
